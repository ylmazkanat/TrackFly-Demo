...
class SurucuYolculukDetay extends StatelessWidget {
  final Map<String, dynamic> journey;

  const SurucuYolculukDetay({super.key, required this.journey});
  ...
  Future<void> _openWhatsApp(BuildContext context ,String message) async {
    try {
      ...
      if (results.isNotEmpty) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Yolcunun telefon numarası bulunamadı!")),
        );
      }
    } catch (e) {
      print('Veritabanı hatası: $e');
  ...
  Future<String?> _getPassengerPhoneNumber(String username) async {
    try {
      final conn = await DatabaseHelper.connect();
      final results = await conn.query(
        "SELECT phone FROM users WHERE username = ?",
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data != null && data['candidates'] != null && data['candidates'].isNotEmpty && data['candidates'][0]['content'] != null && data['candidates'][0]['content']['parts'] != null && data['candidates'][0]['content']['parts'].isNotEmpty) {
          ...
          return prediction; // Return the AI-generated prediction directly
        } else {
          print('Gemini API response format unexpected: $data');
          return 'Rötar tahmini yapılamadı.';
        }
      } else {
        print('Gemini API hatası: ${response.body}');
        return 'Rötar tahmini yapılamadı.';
      }
    } catch (e) {
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 10),
                FutureBuilder<String?>(
                  future: _getPassengerPhoneNumber(journey['yolcu_username']),
                  builder: (context, snapshot) {
                    if (snapshot.connectionState == ConnectionState.waiting) {
                      return const CircularProgressIndicator();
                    }
                    final phone = snapshot.data ?? "Telefon numarası bulunamadı";
                    return GestureDetector(
                      onTap: () => _openWhatsApp(context,""),
                      child: _buildDetailRow(Icons.phone, "Yolcu Tel:", phone, statusColor: AppStyles.buttonColor),
                    );
                  },
                ),
                const SizedBox(height: 10),
                ...
                const SizedBox(height: 10),
                _buildDetailRow(Icons.timer_off, "Bitiş Saati:", _formatDateTime(actualEndTime)),
                const Divider(height: 30, thickness: 1),
                _buildDetailRow(Icons.info, "Durum:", journey['status'] ?? "Belirtilmemiş", statusColor: AppStyles.iconColorOrange),
                const SizedBox(height: 20),
                _buildRemainingTimeRow(actualEndTime),
                const SizedBox(height: 20),
                ...
                FutureBuilder<String>(
                  future: _getWeatherForecast(actualEndTime),
                  builder: (context, snapshot) {
                    if (snapshot.connectionState == ConnectionState.waiting) {
                      return const Center(child: CircularProgressIndicator());
                    }
                    ...
                    children: [
                      Expanded(
                        child: ElevatedButton.icon(
                          onPressed: () => _openWhatsApp(context,"Selam Havaalanı servisini bekliyorum."),
                          ...
                          label: const Text("Bekliyorum"),
                        ),
                      ),
                      const SizedBox(width: 10),
                      Expanded(
                        child: ElevatedButton.icon(
                          onPressed: () => _openWhatsApp(context,"Merhaba, gecikme yaşadım."),
                          style: ElevatedButton.styleFrom(
                          ...
                          icon: const Icon(Icons.message, color: AppStyles.textColorWhite),
                          label: const Text("Yolcuya Mesaj Gönder"),
                        ),
                      ),
                    ],
                  ),
  ...
  Widget _buildRemainingTimeRow(DateTime actualEndTime) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        const Row(
        ...
        Text(
          _calculateRemainingTime(actualEndTime),
          style: TextStyle(
            fontSize: 16,
            color: actualEndTime.isBefore(DateTime.now()) ? AppStyles.buttonColor : AppStyles.textColor,
          ),
        ),
      ],
    );
